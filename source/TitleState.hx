...
{
	//public static var psychEngineVersion:String = '4.2.1'; //This is also used for Discord RPC
	...
	var easterEggKeys:Array<String> = [
		'SHADOW', 'RIVER', 'SHUBS', 'BBPANZU', 'ANGEL'
	];
	...
	override public function create():Void
	{
		...
		//var versionShit:FlxText = new FlxText(12, ClientPrefs.getResolution()[1] - 44, 0, "Vs Mouse Dev Version v" + psychEngineVersion, 12);
	...
	function startIntro()
	{
		...
		switch(easterEgg.toUpperCase())
		{
			...
			case 'ANGEL':
				gfDance.frames = Paths.getSparrowAtlas('AngelBump');
				gfDance.animation.addByPrefix('danceLeft', 'Angel Title Bump', 24);
				gfDance.animation.addByPrefix('danceRight', 'Angel Title Bump', 24);	
		}
			...
			if(pressedEnter)
			{
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					if (mustUpdate) {
						...
						MusicBeatState.switchState(new MainMenuState()); //MusicBeatState.switchState(new WarningState());
					}
					closedState = true;
				});
	...
	override function beatHit()
	{
		...
		var versionShit:FlxText = new FlxText(12, ClientPrefs.getResolution()[1] - 44, 0, "Vs Mouse Dev Version v" + MouseEngineVer.vsmousedev.join("."), 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		versionShit.updateHitbox();
		FlxTween.tween(versionShit, {alpha: 0}, 1, {ease: FlxEase.circOut, startDelay: 2});
		add(versionShit);
		...
		if(!closedState) {
			...
			{
				case 1:
					createCoolText(['Vs Mouse By'], 15);
				case 2:
					addMoreText('Ulkarizers', 15);
				case 3:
					deleteCoolText();
				case 4:
					createCoolText(['Ayop-Mods-OFC'], -40);
				case 5:
					addMoreText('AngelTubby08', -40);
				case 6:
